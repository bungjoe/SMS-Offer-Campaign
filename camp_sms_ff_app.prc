CREATE OR REPLACE PROCEDURE CAMP_SMS_FF_APP AS
    ...
    CURSOR CUR_ROW IS
           ...
           AND CODE_PROMO IN
               (
                    SELECT MESSAGE_CODE FROM CAMP_CFG_SMS_TEXT WHERE CAMPAIGN_ID = TO_CHAR(DT_CURRENT,'YYMM') AND SMS_TYPE ='APP'
               )
           AND CUID <> 615898;
...
END;
BEGIN
    AP_PUBLIC.CORE_LOG_PKG.PINIT( 'AP_CRM', 'CAMP_SMS_FF_APP');
    DT_CURRENT := SYSDATE;
    ...
    AP_PUBLIC.CORE_LOG_PKG.pStart('UPD:TIME SETTING');
        ...
        where delivery_time >= trunc(dt_current)
          and code_promo in (select message_code from camp_cfg_sms_text where campaign_id = to_char(dt_current,'yymm') and sms_type = 'APP')
